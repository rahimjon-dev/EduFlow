...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-20 h-16 bg-slate-900/30 backdrop-blur-xl border-b border-white/10 px-4 sm:px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative hidden sm:block w-64 md:w-80">
          ...
          <input
            ...
            className="w-full bg-white/10 hover:bg-white/20 border border-white/5 rounded-full pl-9 pr-4 py-2 text-xs text-slate-200 placeholder:text-slate-400 focus:bg-white/20 focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-inner"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={roleRef}>
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-xs font-semibold text-slate-200 transition-colors shadow-2xs"
          >
            <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse" />
            <span className="hidden md:inline text-slate-400">{t('roles.viewAs')}</span>
            <span className="text-indigo-400">{currentUser.role}</span>
            <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
          </button>

          {roleDropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-white/10 py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              <div className="px-3 py-1.5 border-b border-white/10">
                <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{t('roles.switchRole')}</p>
                <p className="text-xs text-slate-400">{t('roles.previewRole')}</p>
              </div>
              <div className="p-1">
                {roles.map((item) => (
                  <button
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="p-1.5 rounded-md bg-white/10">{item.icon}</div>
                      <div>
        ...
        <div className="relative" ref={notifRef}>
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-white/10 py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              <div className="px-4 py-2 border-b border-white/10 flex items-center justify-between">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300">{t('common.notifications')}</h4>
                {notifications.length > 0 && (
                  <span onClick={handleMarkAllRead} className="text-[11px] text-indigo-400 hover:underline cursor-pointer font-medium">{t('common.markAllRead')}</span>
                )}
              </div>
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div key={n.id} className="p-3 hover:bg-white/5 transition-colors">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5 leading-tight">{n.text}</p>
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-2.5 p-1 rounded-lg hover:bg-white/10 transition-colors"
          >
            {currentUser.avatar ? (
              <img
            ...
            <div className="hidden sm:flex flex-col text-left">
              ...
              <span className="text-[11px] text-slate-400 leading-none">{currentUser.role}</span>
            </div>
            <ChevronDown className="w-3.5 h-3.5 text-slate-400 hidden sm:block" />
          </button>

          {profileDropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-white/10 py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
              ...
              <div className="p-1">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-slate-200 hover:bg-white/10 rounded-lg transition-colors"
                >
